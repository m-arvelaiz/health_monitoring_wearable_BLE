...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomHmHdle;                    /**< HealthMonitoringS handle */
...
#define COPY_HEALTHMONITORINGS_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
...
#define COPY_CHARNOTIFY_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for HealthMonitoringS +
   *                                2 for CharWrite +
   *                                2 for CharNotify +
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomHmHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: HM, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: HM \n\r");
  }
  ...
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : CHARNOTIFY \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARWRITE:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomHmHdle, CustomContext.CustomCharwriteHdle, SizeCharwrite, pPayload);
    ...
    case CUSTOM_STM_CHARNOTIFY:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomHmHdle, CustomContext.CustomCharnotifyHdle, SizeCharnotify, pPayload);
